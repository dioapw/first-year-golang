...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		hitungFrekuensi(data, m, max(data, m), &frek)             //to count frequency of max number
		sum = fmt.Sprintf("%s %d %d \n", sum, max(data, m), frek) //to keep the value until the process finished
	}
	fmt.Println(sum)
}
